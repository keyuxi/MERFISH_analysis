function MERFISHProbeDesign(varargin)
    ...
    % Additional modifications by YK, Schnitzer Lab, Stanford, 2020.
    %% ------------------------------------------------------------------------
    % Setup the workspace
    %%-------------------------------------------------------------------------
    ...
    elseif (nargin == 1) && isa(varargin{1}, 'probeDesign')
       ...
       if isempty(fileparts(obj.codebookPath))
        ...
        forbiddenSeqs = obj.forbiddenSeqs;
        
        splitType = obj.splitType; % live in both immediate and regionDesignParameters namespaces
        regionDesignParameters.splitType = obj.splitType;
    ...
    try
        ...
        % 		'OTTables', {'rRNA', [0, 0]});
            ...
            fprintf(logFID, '%s - Designing target regions %s\n', datestr(datetime));
                ...
                targetRegions = trDesigner.DesignTargetRegions(...
                    'splitType', regionDesignParameters.splitType, ...
        ...
        if isfield(codebook(1), 'subpool')
            finalGenesSubpoolNames = {codebook.subpool};
        end
        ...
        if keepGoingFlag
            ...
            for i=1:length(finalIds)
                ...
                localSubpoolName = finalGenesSubpoolNames{i};
                ...
                if ~strcmp(localGeneName, lastGene)
                    ...
                    if ~isempty(tRegionPull) % Check to see if there are no target regions--only used for blanks
                        ...
                        for tR = 1:length(tRegionPull)
                            ...
                            % if split, check if TRegions are paired and correctly spaced
                            if strcmp(splitType, 'split')
                                % paring
                                if mod(tRegion.numRegions, 2) ~= 0
                                   fprintf(1, 'Target regions for splitting %s are not paired!\n', localGeneName);
                                   fprintf(logFID, '%s - Target regions for splitting %s are not paired!\n', datestr(datetime), localGeneName);
                                end
                                
                                % gap spacing
                                if (tRegion.startPos(2) - tRegion.startPos(1)) ~= tRegion.regionLength(1) + 2
                                    fprintf(1, 'Target regions for splitting %s are not correctly spaced!\n', localGeneName);
                                    fprintf(logFID, '%s - Target regions for splitting %s are not correctly spaced!\n', datestr(datetime), localGeneName);
                                end
                            end
                            ...
                            for nR=1:tRegion.numRegions
                                if strcmp(splitType, 'conventional')
                                    ...
                                    % 2 ON bits, smELT style
                                    elseif (sum(barcodes(i, :)) == 2) 
                                        
                                        if tripleHeadedsmELT
                                            possibleReadoutInds = [1,2,mod(nR,2)+1];
                                            possibleReadoutInds = possibleReadoutInds(randperm(3));
                                            localReadouts = possibleReadouts(possibleReadoutInds);
                                        else
                                ...
                                elseif strcmp(splitType, 'split')
                                    % only have 1 local readout (barcode)
                                    localReadouts(1) = possibleReadouts(1);
                                ...
                                end % 'conventional' vs 'split'

                                p = p + 1;

                            end % nR loop, for every oligo
